...
unsigned int loadVerticalSeam(Pixel image[][MAX_HEIGHT], unsigned int start_col, unsigned int width, unsigned int height, unsigned int seam[]) {
  ...
  unsigned int totalEnergy = energy(image, start_col, 0, width, height); // initial totalEnergy
  seam[0] = start_col;
for (unsigned int row = 1; row < height; row++) {
 int z = 1; //let 1 = forward, 2 = right, 3 = left
  currMin = energy(image, start_col, row, width, height); //set min as directly forward
  if (start_col + 1 <= width) {
    if(energy(image, start_col+1, row, width, height) < currMin) { //incr col by 1
      currMin = energy(image, start_col+1, row, width, height);
      z = 2;   
  }
  }
  if (start_col != 0) {
...
//   // TODO: implement (part 2)
// need to pass copy of size seam array
unsigned int* seamCopy = new unsigned int[MAX_HEIGHT];
unsigned int minSeam = loadVerticalSeam(image, 0, width, height, seam);
  for (unsigned int col = 1; col < width; col++) {
    if(loadVerticalSeam(image, col, width, height, seamCopy) < minSeam) {
      minSeam = loadVerticalSeam(image, col, width, height, seam);
    }
  }
}
...
//   // TODO: implement (part 2)
  for (unsigned int row = 0; row < height; row++) {
    for (unsigned int col = verticalSeam[row]; col < width-1; col++) {
